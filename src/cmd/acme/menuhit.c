 ...
 */
static int
menuscan(Image *m, Menu *menu, int but, Mousectl *mc, Rectangle textr, int off, int lasti, Image *save, int *lbut)
{
	int i, nb, p, state;
	paintitem(m, menu, textr, off, lasti, 1, save, nil);
	state = 0;
	for(readmouse(mc); mc->m.buttons & (1<<(but-1)); readmouse(mc)){
		/* but is always pressed. When a second button is
			pressed, make it possible to "cancle" the action by
			pressing the third button. If the second button is
			released whithout pressing the third, record that click */
		p = mc->m.buttons;
		for(nb = 0; p; p&=(p-1)) // Kernighans algorithem
			nb++;
		if(nb > 2) {
			*lbut = mc->m.buttons;
			break;
...
int
acme_menuhit(int but, Mousectl *mc, Menu *menu, int *buts)
{
	...
	while(mc->m.buttons & (1<<(but-1))){
		lasti = menuscan(b, menu, but, mc, textr, off, lasti, save, buts);
		if(lasti >= 0)
			break;
